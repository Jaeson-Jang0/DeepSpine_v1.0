...
class Trainer: 
    ...
    def _train_session(self, session, kinematics, ees): # Synthetic # 230227
...
#         kinematics, emg, ees, meta, afferents, eesIntegratedAfferents = x #####
...
#         optim_readout.step()
    ...
    def _train_iteration(self, iteration):
        ...
        for session in list_session:
            for kinematics, emg, ees, meta, afferents, eesIntegratedAfferents in train_loader: # Synthetic
                ...
                loss = self._train_session(session, kinematics, ees) # Synthetic
            ...
            with torch.no_grad():
                if self.with_weight_clipping:
                    ...
                    self.core.emg2aff_connection.weight = torch.nn.parameter.Parameter(abs(self.core.emg2aff_connection.weight))                   
                ...
                if self.with_bias_clipping: ##### 230531
                    ...
                    self.core.emg2aff_connection.bias = torch.nn.parameter.Parameter(abs(self.core.emg2aff_connection.bias))
    ...
    def _valid_session(self, iteration, session, is_save=False, with_pred=False):  
        ...
        batch_id = 0
        for kinematics, emg, ees, meta, afferents, eesIntegratedAfferents in valid_loader:           
...
#             print(kinematics.shape, emg.shape, ees.shape, meta.shape, afferents.shape)
#             print(meta[0,0,:])
#             print("Max: ", torch.max(ees[0,0,:]))
#             print("Loc: ", torch.squeeze(torch.argwhere(ees[0,0,:])))
#             temp = ees[0,0,:]
#             print("Val: ", temp[temp>0])
#             print(meta)
            
#             ees = torch.zeros((kinematics.shape[0], len(ch_list), kinematics.shape[2])) ##### sling
#             for idx in range(meta.shape[0]):
#                 if torch.sum(meta[idx,:]) > 0: ##### 230314
#                     elec, amp, freq = meta[idx,0,0], meta[idx,0,1], meta[idx,0,2]
#                     elec = elec.detach().cpu()
#                     amp = amp.detach().cpu()
#                     freq = freq.detach().cpu()

# #                     elec_idx = np.argwhere(ch_list==elec.numpy())[0][0]
#                     elec_idx = 0 ##### Synthetic
...
#                     ees[idx, elec_idx, :] = ees_template
            ...
            # [batch x dim x time]
#             ees = ees.to(self.device)           
#             print('EES: ', ees.shape, torch.max(ees))
#             print(torch.squeeze(torch.argwhere(ees[0,0,:])))
#             temp = ees[0,0,:]
#             print(temp[temp>0])
...
#             print("Response:", kinematics.shape, ees.shape, predicted_kinematics.shape)
            ...
            for trial_idx in range(kinematics.shape[0]):
                crop_temp = []
                for t_idx in range(int(tdur/tbin)):
                    temp = np.mean(kinematics_np[trial_idx, :, t_idx:(t_idx+1)], axis=1) ##### Synthetic ##### 230316
            ...
            loss = self.criterion(predicted_kinematics[:,:,(int(t0/tbin/2)-2):(int(tdur/tbin)-2)], kinematics_crop[:,:,int(t0/tbin/2):int(tdur/tbin)], reduc='none') ##### 230313
...
#                 'emg': emg,
                ...
                'loss': loss,
                'meta': metas,
#                 'afferents_ch': predicted_neural[0],
#                 'afferents_ch_ees': predicted_neural[1],
#                 'afferents': predicted_neural[2],                
#                 'exsF': predicted_neural[3],
#                 'exsE': predicted_neural[4],
#                 'IaisF': predicted_neural[5],
#                 'IaisE': predicted_neural[6],
#                 'mnsF': predicted_neural[7],
#                 'mnsE': predicted_neural[8],
                'neural': list_neural
            }
    ...
    def train(self):
        ...
        for iteration in range(self.start_iter, self.max_iter):
            ...
            if (iteration+1) % self.eval_period == 0:
                ...
                with torch.no_grad():
                    ##### 230209: Saving prediction only for last iteration
                    if (iteration+1) % 10 == 0: #== self.max_iter: % 100
                        valid_loss = self._valid_iteration(iteration, is_save=True, with_pred=True)
                    else:

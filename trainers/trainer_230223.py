# Revised from "trainer_230223": To apply to treadmill date
...
class Trainer: 
    def __init__(self, 
        ...
        print('Batch# : ', len(self.batch_iterators))
...
#             ees[idx, elec_idx*njoint:(elec_idx+1)*njoint, :] = ees_template
        ...
        predicted_kinematics, predicted_neural = self.core(kinematics, ees)
...
#         predicted_emg = readout(motor_neurons)  # [batch x dim x time]
        ...
        print('Pred: ', predicted_kinematics.shape, kinematics.shape)
        loss = self.criterion(predicted_kinematics[:,:,501:], kinematics[:,:,501:])
...
#         optim_readout.step()
    ...
    def _train_iteration(self, iteration):
        ...
        for session in list_session:
            ...
            print('Loss list: ', len(list_loss), loss.shape)
    ...
    def _valid_session(self, iteration, session, is_save=False, with_pred=False):  
        ...
        for kinematics, meta in valid_loader: ##### 230216
            ...
            print(kinematics[0,0,0], kinematics.shape)
            batch_id += 1
...
#             predicted_emg = readout(motor_neurons)  # [batch x dim x time]
            ...
            loss = self.criterion(predicted_kinematics[:,:,501:], kinematics[:,:,501:], reduc='none') ##### 230216
        ...
        for iteration in range(self.start_iter, self.max_iter):
            ...
            if (iteration+1) % self.eval_period == 0:
                ...
                with torch.no_grad():
                    ...
                    if (iteration+1) % 200 == 0: #== self.max_iter:

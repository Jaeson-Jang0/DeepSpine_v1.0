# Revised from "trainer_230926_unified_t25_mid__05_1_usingInference": To add xy encoding
# Revised from "trainer_230926_unified_t25_mid__05_1_usingInference": To reduce computation when EMG is unnecessary
...
#         self.core.to(self.device)
        ...
        if 'resume' in trainer_cfg.keys():
            ...
            #             self._load_model(state) ##### v231010             
        ...
        ##### xy
        with open('xy_loc_Phoenix_202309.json', 'r') as json_file:
            self.xy_loc = json.load(json_file) 
        
    
    def _load_model(self, state):
        self.core.load_state_dict(state['core'])
#         self.core.load_state_dict(state) ##### v231010
    ...
    def _train_session(self, session, kinematics, emg, meta): ##### 230713, 230703: If synthetic, meta is ees; 230227       
        ...
        if self.datatype == 'synthetic':
            ...
            ees = torch.zeros((kinematics.shape[0], 16, kinematics.shape[2])) ##### xy
...
#                     elec_idx = np.argwhere(ch_list==elec.numpy())[0][0]
                    elec_x_idx = self.xy_loc[str(int(elec.numpy()))]['x_step'] ##### xy
                    elec_y_idx = self.xy_loc[str(int(elec.numpy()))]['y_step']
...
#                     ees[idx, elec_idx, :] = ees_template
                    ees[idx, elec_x_idx, :] = ees_template ##### xy
                    ees[idx, elec_y_idx+8, :] = ees_template
...
#                 loss = self.criterion(predicted_kinematics[:,:,(int(t0/tbin/2)-2):(int(tdur/tbin)-2)], kinematics_crop[:,:,int(t0/tbin/2):int(tdur/tbin)])
                loss = self.criterion(predicted_kinematics[:,:,0:(int(tdur/tbin)-2)], kinematics_crop[:,:,2:int(tdur/tbin)])
...
#                 loss = self.criterion(predicted_emg[:,:,0:(int(t0/tbin)-2)], emg_crop[:,:,2:int(t0/tbin)]) ##### mid
                loss = self.criterion(predicted_emg[:,:,0:(int(tdur/tbin)-2)], emg_crop[:,:,2:int(tdur/tbin)]) ##### 230926
            ...
            ##### 230731: Training using both EMG and kinematics
                ...
                loss_kine = self.criterion(predicted_kinematics[:,:,0:(int(tdur/tbin)-2)], kinematics_crop[:,:,2:int(tdur/tbin)]) ##### _05_1
                loss_emg = self.criterion(predicted_emg[:,:,0:(int(tdur/tbin)-2)], emg_crop[:,:,2:int(tdur/tbin)])
...
#                         self.core.mns2aff_connection.bias = torch.nn.parameter.Parameter(abs(self.core.mns2aff_connection.bias)) ##### 230727
    ...
    def _valid_session(self, iteration, session, is_save=False, with_pred=False):  
        ...
        for kinematics, emg, meta in valid_loader: ##### 230713, 230216
            ...
            ees = torch.zeros((kinematics.shape[0], 16, kinematics.shape[2])) ##### xy; sling
            for idx in range(meta.shape[0]):
                if torch.sum(meta[idx,:]) > 0: ##### 230314
                    elec, amp, freq = meta[idx,0], meta[idx,1], meta[idx,2]
                    elec = elec.detach().cpu()
...
#                     elec_idx = np.argwhere(ch_list==elec.numpy())[0][0]
        ...
        #                     ees[idx, elec_idx, :] = ees_template
            ...
            if self.training_target == 'kine':
                ...
                loss = self.criterion(predicted_emg[:,:,0:(int(tdur/tbin)-2)], emg_crop[:,:,2:int(tdur/tbin)]) ##### mid
...
#         import ipdb; ipdb.set_trace()
    ...
    def _save_checkpoint(self, iteration, save_best=False):
        ...
        torch.save(state, filename) ##### Original
#         torch.save(self.core.state_dict(), filename) ##### For inference model

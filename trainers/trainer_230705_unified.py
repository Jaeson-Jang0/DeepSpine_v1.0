# Revised from "trainer_230703_unified": To apply to training for EMG
...
#         self.core.to(self.device)
    ...
    def _train_session(self, session, kinematics, meta, emg): ##### 230705; 230703: If synthetic, meta is ees; 230227       
        # [batch x dim x time]
        ...
        if self.datatype == 'synthetic':
            ...
            if self.outputtype == 'kinematics':
                loss = self.criterion(predicted_kinematics[:,:,int(t0/2):], kinematics[:,:,int(t0/2):predicted_kinematics.shape[2]])
            elif self.outputtype == 'emg': ##### 230705
                loss = self.criterion(predicted_emg[:,:,int(t0/2):], emg[:,:,int(t0/2):predicted_emg.shape[2]])
...
#         optim_readout.step()
    ...
    def _train_iteration(self, iteration):
        ...
        for session in list_session:
            for kinematics, emg, meta in train_loader:
                ...
                loss = self._train_session(session, kinematics, meta, emg) ##### 230705
    ...
    def _valid_session(self, iteration, session, is_save=False, with_pred=False):  
        ...
        batch_id = 0
        for kinematics, emg, meta in valid_loader: ##### 230216
            from datetime import datetime
            now = datetime.now()
            current_time = now.strftime("%H:%M:%S")            
            ...
            if self.outputtype == 'kiematics':
                loss = self.criterion(predicted_kinematics[:,:,int(t0/2):], kinematics[:,:,int(t0/2):predicted_kinematics.shape[2]], reduc='none')

                print(predicted_kinematics.shape, loss)
            elif self.outputtype == 'emg': ##### 230705
                loss = self.criterion(predicted_emg[:,:,int(t0/2):], emg[:,:,int(t0/2):predicted_emg.shape[2]], reduc='none')
                
                print(predicted_emg.shape, loss)
           
            if is_save:
                list_loss.append(loss)
                list_meta.append(meta)
                
                if self.outputtype == 'emg': list_emg.append(emg)
        ...
        if is_save and with_pred: # 230209: Saving prediction only for last iteration
            ...
            if self.outputtype == 'emg':
                state_emg = {'emg': emg}
                state = {**state, **state_emg}

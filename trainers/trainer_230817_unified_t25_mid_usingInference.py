# Revised from "trainer_230817_unified_t25_mid_usingInference": Using preprocessed emg and angle from inference model
...
#                 loss_emg = self.criterion(predicted_emg[:,:,0:(int(t0/tbin)-2)], emg_crop[:,:,2:int(t0/tbin)])
                ...
                # For EES sling
                loss_kine = self.criterion(predicted_kinematics[:,:,0:(int((tdur+t0)/2/tbin)-2)], kinematics_crop[:,:,2:int((tdur+t0)/2/tbin)])
                loss_emg = self.criterion(predicted_emg[:,:,0:(int((tdur+t0)/2/tbin)-2)], emg_crop[:,:,2:int((tdur+t0)/2/tbin)])
    ...
    def _train_iteration(self, iteration):
        ...
        for session in list_session:
            for kinematics, emg, meta in train_loader: ##### 230713
                ...
                ##### usingInference: No preprocessing

...
#         self.core.to(self.device)
    ...
    def _train_session(self, session, kinematics, emg, meta): ##### 230713, 230703: If synthetic, meta is ees; 230227       
        # [batch x dim x time]
        emg = emg[:, np.array(self.muscle_list), :] ##### 230718, 230713
...
#                 loss_emg = self.criterion(predicted_emg[:,:,0:(int(t0/tbin)-2)], emg_crop[:,:,2:int(t0/tbin)])
                ...
                # For EES sling # from250
                loss_kine = self.criterion(predicted_kinematics[:,:,int(t0/2/tbin)+0:(int((tdur+t0)/2/tbin)-2)], kinematics_crop[:,:,int(t0/2/tbin)+2:int((tdur+t0)/2/tbin)])
                loss_emg = self.criterion(predicted_emg[:,:,int(t0/2/tbin)+0:(int((tdur+t0)/2/tbin)-2)], emg_crop[:,:,int(t0/2/tbin)+2:int((tdur+t0)/2/tbin)])

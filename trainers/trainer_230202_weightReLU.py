...
#         print(kinematics.shape, emg.shape, ees.shape, meta.shape)
    ...
    def _train_iteration(self, iteration):
        ...
        for session in list_session:
            ...
            ##### v230302: ReLu on weight            
            with torch.no_grad():
                wReLU = nn.ReLU()
                
                self.core.IaF2mnFIaiF_connection.weight = torch.nn.parameter.Parameter(wReLU(self.core.IaF2mnFIaiF_connection.weight))
                self.core.IIF2exFIaiF_connection.weight = torch.nn.parameter.Parameter(wReLU(self.core.IIF2exFIaiF_connection.weight))
                self.core.exF2mnF_connection.weight = torch.nn.parameter.Parameter(wReLU(self.core.exF2mnF_connection.weight))
                self.core.IaiF2IaiEmnE_connection.weight = torch.nn.parameter.Parameter(-wReLU(-self.core.IaiF2IaiEmnE_connection.weight))
                
                self.core.IaE2mnEIaiE_connection.weight = torch.nn.parameter.Parameter(wReLU(self.core.IaE2mnEIaiE_connection.weight))
                self.core.IIE2exEIaiE_connection.weight = torch.nn.parameter.Parameter(wReLU(self.core.IIE2exEIaiE_connection.weight))
                self.core.exE2mnE_connection.weight = torch.nn.parameter.Parameter(wReLU(self.core.exE2mnE_connection.weight))
                self.core.IaiE2IaiFmnF_connection.weight = torch.nn.parameter.Parameter(-wReLU(-self.core.IaiE2IaiFmnF_connection.weight))
    ...
    def train(self):
        ...
        for iteration in range(self.start_iter, self.max_iter):
            ...
            if (iteration+1) % self.eval_period == 0:
                ...
                with torch.no_grad():
                    valid_loss = self._valid_iteration(iteration, is_save=True) ##### # False

                    log= ''
                    for session, (t_loss, v_loss) in enumerate(zip(train_loss, valid_loss)):
                        log += '[sess: %d][iter: %d]: %02.3f | %02.3f \n' % (session, iteration, mean(t_loss), torch.mean(v_loss)) #####                

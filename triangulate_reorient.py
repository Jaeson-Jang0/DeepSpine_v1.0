...
def triangulate(config,
    ...
    print("Calib: ", calib_folder)
    print("Video: ", video_folder)
    print("Pose: ", pose_folder)
    cam_names = sorted(fname_dict.keys())
    ...
    # print(fname_dict, offsets_dict, cam_names)
    out = load_pose2d_fnames(fname_dict, offsets_dict, cam_names)
    
    print('Asdf')
    print(out.keys())
    print(out['points'].shape, out['scores'].shape)
    ...
    print(config['triangulation']['optim'], config['triangulation']['ransac'])
    new_coor = []
	...
	##### Setting directory
        save_name = '230810_3_Z-Jaeson-2023-07-26'
        new_dir = '/home/jjang50/DeepLabCut/' + save_name + '/labeled-data/'
        cam_list = ['Block0003_Cam1/', 'Block0003_Cam2/', 'Block0003_Cam3/']
	
        for cam_idx, cam_name in enumerate(cam_list):
            df_cam = pd.read_csv(new_dir+cam_name+'CollectedData_Jaeson.csv')
            df_info = df_cam.iloc[2:, 3:].to_numpy()
            df_info = np.array(df_info).astype(float)
            
            df_shaped = df_info.reshape(10, 6, 2)
            
            # print(df_cam)
            # print(df_info, df_info.shape)
            # print(df_shaped, df_shaped.shape)
            
            new_coor.append(df_shaped)
	
        new_points_2d = np.stack(new_coor, axis=0)
        new_points_shaped = new_points_2d.reshape(n_cams, new_points_2d.shape[1]*new_points_2d.shape[2], 2)

        print(new_points_2d.shape, new_points_shaped.shape)
        ...
        if config['triangulation']['ransac']:
            ...
            new_points_3d_init = cgroup.triangulate(new_points_shaped, progress=True)
            np.savetxt('corner_' + save_name + '.csv', new_points_3d_init, delimiter=',')
            print(new_points_3d_init, new_points_3d_init.shape)
            points_3d_init = cgroup.triangulate(points_shaped, progress=True)
                    
        print('Pre: ', points_3d_init.shape)
        new_points_3d_init = new_points_3d_init.reshape((new_points_2d.shape[1], new_points_2d.shape[2], 3))
        print(new_points_3d_init.shape)
        points_3d_init = points_3d_init.reshape((n_frames, n_joints, 3))
        print('Post: ', points_3d_init.shape)
        ...
        if np.sum(c) < 20:
            ...
            print("After optim: ", points_3d.shape)

...
class StackedGRU_231027_forInfer_300ms(nn.Module):
    ...
    def __init__(self,
                 ...
                 rdo_out_channels,
                 rdo_kernel_size,
                 ini_kine, ini_kine_dir, ##### forInfer
                 rdo_groups=1,
                 rdo_activation='none',
                 offset=0,
                 activation='relu',
        ...
        super(StackedGRU_231027_forInfer_300ms, self).__init__()
        
        self.ini_kine = ini_kine ##### forInfer
        self.ini_kine_dir = ini_kine_dir
...
#             hidden_size = self.hidden_size[0]
    ...
    def forward(self, theta): # kinematics, ees, with_ees, verbose=False):
        ##### 231027, 230307
        t_total, t0 = 850, 500
...
#         ees = ees.to(self.device)
        ...
        for t in range(n_bin):
            ...
            kine_crop[:,:,t] = torch.mean(kinematics[:,:,t_start:t_start+t_bin], dim=2) ##### forInfer
...
#                 kinematics_bin_emg = torch.cat((torch.tile(emg_1, (1,1,25)), torch.tile(emg, (1,1,25))), dim=2) 
                kinematics_bin_emg = torch.cat((emg_1.repeat(1,1,25), emg.repeat(1,1,25)), dim=2) ##### forInfer
...
#             hidden_size = self.hidden_size[0]
                for gru in self.grus:
                    ...
                    h = gru(afferents_t[:,:,i], hx) ##### forInfer
...
#         y_pred = emg_stack[:,:,22] - emg_stack[:,:,18] ##### v231010   
#         y_base = kine_crop[:,:,24] - kine_crop[:,:,20]    
        
        y_pred = emg_stack[:,:,30] - emg_stack[:,:,18] ##### 300ms, v231010   
        y_base = kine_crop[:,:,32] - kine_crop[:,:,20]    
            
        y_cal = y_pred - y_base                  
    
        return y_cal #emg_stack, emg_stack, [] ##### 231027
#         if verbose:
#             if self.offset > 0:

...
class SpinalCordCircuit_230703_unified_t25_4(nn.Module):
    def __init__(self,
            emb_in_channels,
            emb_out_channels,
            emb_kernel_size,
        ...
        super(SpinalCordCircuit_230703_unified_t25_4, self).__init__()
...
#         self.register_buffer('mn_connectivity', torch.BoolTensor([True, True, False])) ##### 230531: Already negative
        ...
        self.rdo_mns2kine = CausalConv1d(rdo_in_channels, rdo_out_channels, kernel_size=rdo_kernel_size, groups=rdo_groups, pad=rdo_kernel_size-1) ##### v1
...
#             self.rdo_norm = LayerNorm(dim=2, affine_shape=[rdo_groups, int(rdo_out_channels/rdo_groups), 1])
        ...
        self.kine2aff_connection = conv1d(rdo_out_channels, 2*(Ia_neurons+II_neurons), kernel_size=1, groups=1) ##### v1; 230630
        torch.nn.init.trunc_normal_(self.kine2aff_connection.weight, a=0.0, b=float('inf')) ##### v1; 230531
        ...
        torch.nn.init.trunc_normal_(self.kine2aff_connection.bias, a=0.0, b=float('inf')) ##### v1; 230531
...
#         t_total = kinematics.shape[2]
        ...
        for t in range(n_bin): ##### t25
            ...
            ##### 230309
            if t > 17: ##### t25
                kinematics_bin_fb = torch.cat((torch.tile(kine_1, (1,1,25)), torch.tile(kine, (1,1,25))), dim=2) 
            ...
            if t == 0:            
                ...
                Ia_flxs_t = Ia_flxs #+ fb_Ia_flxs ##### v4 # withKineFB
                II_flxs_t = II_flxs #+ fb_II_flxs
                Ia_exts_t = Ia_exts #+ fb_Ia_exts
                II_exts_t = II_exts #+ fb_II_exts
            elif t == 20:
                Ia_flxs_t = torch.cat((Ia_flxs[:, :, :25], Ia_flxs_kine[:,:,25:]), dim=2) #+ fb_Ia_flxs
                II_flxs_t = torch.cat((II_flxs[:, :, :25], II_flxs_kine[:,:,25:]), dim=2) #+ fb_II_flxs
                Ia_exts_t = torch.cat((Ia_exts[:, :, :25], Ia_exts_kine[:,:,25:]), dim=2) #+ fb_Ia_exts
                II_exts_t = torch.cat((II_exts[:, :, :25], II_exts_kine[:,:,25:]), dim=2) #+ fb_II_exts                 
#                 coef = t/int(t0/2)
...
#                 II_exts_t = (1-coef)*II_exts + coef*II_exts_kine + fb_II_exts
            ...
            kine = self.rdo_mns2kine(mns) ##### v1, v3 # BxCxT
            emg = kine
...
#                 emg = self.rdo_activation(emg)
            ...
            ##### 230209: Somatosensory feedback
            feedback = self.kine2aff_connection(kine) ##### v1

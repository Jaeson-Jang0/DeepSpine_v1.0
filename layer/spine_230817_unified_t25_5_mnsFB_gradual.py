...
class SpinalCordCircuit_230817_unified_t25_5_mnsFB_gradual(nn.Module):
    def __init__(self,
            muscle_list, kine_list, ds_kine_list, ang_kine_list, ##### 230817
        ...
        super(SpinalCordCircuit_230817_unified_t25_5_mnsFB_gradual, self).__init__()
        ...
        for t in range(n_bin): ##### t25
            ...
            for ds_idx in range(self.n_ds):
                ...
                if t == 0: ##### mnsFB, 230718            
                    ...
                    Ia_exts_t = (Ia_exts)
                    II_exts_t = (II_exts)
                elif t < int(t0/t_int/2): ##### 230817; 230718
                ...
                elif t >= int(t0/t_int/2) and t < int(t0/t_int): ##### 230718
...
#                     II_exts_t = torch.cat((II_exts[:, :, :t_int], II_exts_kine[:,:,t_int:]), dim=2) #+ fb_II_exts                 
                    coef = (t-int(t0/t_int/2))/(int(t0/t_int)-int(t0/t_int/2))
                ...
                emg = self.rdo_mns2emg_list[ds_idx](mns) # BxCxT
#                 kine = self.rdo_mns2kine_list[ds_idx](mns)   
                ...
                if ds_idx == 0:
                    ...
                    emg_t = torch.cat([emg_t, emg], dim=1)
                ...
                if ds_idx == self.n_ds-1:
                    if t == 0:
                        ...
                        emg_stack = torch.zeros(torch.cat((torch.tensor(emg_t.shape[:-1]), torch.tensor([n_bin]))).tolist()).to(device)
                        kine_stack = torch.zeros(torch.cat((torch.tensor(kine_t.shape[:-1]), torch.tensor([n_bin]))).tolist()).to(device)

                    #### Saving for each t
                    emg_stack[..., t:t+1] = emg_t

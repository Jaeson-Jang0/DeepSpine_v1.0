...
#             in_layer = self.activation(in_layer[:,EI].sum(1)) - self.activation(in_layer[:,~EI].sum(1)) # [BxN]
            in_layer = self.activation(in_layer[:,EI].sum(1) - in_layer[:,~EI].sum(1)) # [BxN] ##### 230531_1act
        else:
        ...
        return torch.stack(hiddens[1:], dim=2), in_layer.unsqueeze(2), hx.unsqueeze(2) ##### 230531 # exclude initial hidden state


class SpinalCordCircuit_230531_withKineFB_1act(nn.Module):
    def __init__(self,
            emb_in_channels,
            emb_out_channels,
            emb_kernel_size,
        ...
        super(SpinalCordCircuit_230531_withKineFB_1act, self).__init__()
...
#             print("Post: ", IaF2mnF.shape)
            ...
            exsF, _, _ = self.ex_flxs([IIF2exF], init_exF) ##### 230531
            exF2mnF = self.exF2mnF_connection(exsF)

            exsE, _, _ = self.ex_exts([IIE2exE], init_exE)
            exE2mnE = self.exE2mnE_connection(exsE)       

            # compute inhibitory neurons
#             print("init_IaiE: ", init_IaiE)
            IaisF, _, _ = self.Iai_flxs([IaF2IaiF, IIF2IaiF, torch.unsqueeze(init_IaiE,2)], init_IaiF, self.Iai_connectivity) ##### 230531
            IaisE, _, _ = self.Iai_exts([IaE2IaiE, IIE2IaiE, torch.unsqueeze(init_IaiF,2)], init_IaiE, self.Iai_connectivity)   

# Revised from "spine_230605_simpleAct": Removing complicated structure for dynamic gain control
...
#         temp = xs# + [init_h.unsqueeze(2)]
        temp = torch.permute(torch.cat(xs, 2), (0,2,1))

        if EI is not None:  
            in_layer = self.activation(temp[:,EI].sum(1) - temp[:,~EI].sum(1)) # [BxN]
        else:
            in_layer = self.activation(temp.sum(1))
        ...
        return torch.stack(hiddens[1:], dim=2), in_layer.unsqueeze(2), hx.unsqueeze(2)


class SpinalCordCircuit_230606_simpleActReal(nn.Module):
    def __init__(self,
            emb_in_channels,
            emb_out_channels,
            emb_kernel_size,
        ...
        super(SpinalCordCircuit_230606_simpleActReal, self).__init__()
...
#     def forward(self, x, ees, verbose=False): # 230209
...
#             print("Post: ", IaF2mnF.shape)
            
            # compute excitatory neurons
            exsF, exsF_inlayer, exsF_hx = self.ex_flxs([IIF2exF], init_exF) ##### 230531
            exF2mnF = self.exF2mnF_connection(exsF)

            exsE, exsE_inlayer, exsE_hx = self.ex_exts([IIE2exE], init_exE)
            exE2mnE = self.exE2mnE_connection(exsE)       

            # compute inhibitory neurons
            IaisF, IaisF_inlayer, IaisF_hx = self.Iai_flxs([IaF2IaiF, IIF2IaiF, torch.unsqueeze(init_IaiE,2)], init_IaiF, self.Iai_connectivity) ##### 230531
            IaisE, IaisE_inlayer, IaisE_hx = self.Iai_exts([IaE2IaiE, IIE2IaiE, torch.unsqueeze(init_IaiF,2)], init_IaiE, self.Iai_connectivity)   
            ...
            ##### 230531
            mnsF, mnsF_inlayer, mnsF_hx = self.mn_flxs([IaF2mnF, exF2mnF, IaiE2mnF], init_mnF, self.mn_connectivity)
            mnsE, mnsE_inlayer, mnsE_hx = self.mn_exts([IaE2mnE, exE2mnE, IaiF2mnE], init_mnE, self.mn_connectivity)
...
#             print("Asdf: ", mnsF_hx.shape, mnsF.shape, mnsF_x.shape, mnsF_g.shape, mnsF_gpre.shape, mnsF_inlayerpre.shape)
...
#             print(exsF.shape, Ia_flxs_t.shape, IaF2mnF.shape) 
            
            ##### 230523
#             Ia_flxs_t = torch.mean(Ia_flxs_t, dim=2, keepdim=True)
...
#                 print(mnsF_x_stack.shape, exsF_x_stack.shape, IaisF_x_stack.shape)
                ##### 230605
...
#             print("Pre: ", Ia_flxs_t.shape)
            Ia_flxs_t = torch.mean(Ia_flxs_t, dim=2, keepdim=True) ##### 230531: Just moved
            II_flxs_t = torch.mean(II_flxs_t, dim=2, keepdim=True)

            Ia_exts_t = torch.mean(Ia_exts_t, dim=2, keepdim=True)
            II_exts_t = torch.mean(II_exts_t, dim=2, keepdim=True)
#             print("Post: ", Ia_flxs_t.shape)

#             if t == 0 or t == n_bin-1:
#                 print(t, emg_stack.shape)
...
#                 return emg_stack, [afferents_ch, afferents_ch_ees, afferents, exsF_mean, exsE_mean, IaisF_mean, IaisE_mean, mnsF_mean, mnsE_mean]
#                 return emg_stack, [afferents_ch, afferents_ch_ees, afferents, exsF_stack, IaisF_stack, mnsF_stack, exsE_stack, IaisE_stack, mnsE_stack, Ia_flxs_t_stack, II_flxs_t_stack, Ia_exts_t_stack, II_exts_t_stack, IaF2mnF_stack, IaF2IaiF_stack, IIF2exF_stack, IIF2IaiF_stack, IaE2mnE_stack, IaE2IaiE_stack, IIE2exE_stack, IIE2IaiE_stack] ##### 230523
#                 return emg_stack, [afferents_ch, afferents_ch_ees, afferents, exsF_stack, IaisF_stack, mnsF_stack, exsE_stack, IaisE_stack, mnsE_stack, Ia_flxs_t_stack, II_flxs_t_stack, Ia_exts_t_stack, II_exts_t_stack, IaF2mnF_stack, IaF2IaiF_stack, IIF2exF_stack, IIF2IaiF_stack, IaE2mnE_stack, IaE2IaiE_stack, IIE2exE_stack, IIE2IaiE_stack, exF2mnF_stack, IaiE2mnF_stack, exE2mnE_stack, IaiF2mnE_stack] ##### 230523
...
#                 return emg_stack, [afferents_ch, afferents_ch_ees, afferents, exsF_stack, IaisF_stack, mnsF_stack, exsE_stack, IaisE_stack, mnsE_stack, Ia_flxs_t_stack, II_flxs_t_stack, Ia_exts_t_stack, II_exts_t_stack, IaF2mnF_stack, IaF2IaiF_stack, IIF2exF_stack, IIF2IaiF_stack, IaE2mnE_stack, IaE2IaiE_stack, IIE2exE_stack, IIE2IaiE_stack, exF2mnF_stack, IaiE2mnF_stack, exE2mnE_stack, IaiF2mnE_stack, Ia_flxs_stack, II_flxs_stack, Ia_exts_stack, II_exts_stack, fb_Ia_flxs_stack, fb_II_flxs_stack, fb_Ia_exts_stack, fb_II_exts_stack, mnsF_inlayer_stack, mnsF_hx_stack, mnsE_inlayer_stack, mnsE_hx_stack] ##### 230531
#                 return emg_stack, [afferents_ch, afferents_ch_ees, afferents, exsF_stack, IaisF_stack, mnsF_stack, exsE_stack, IaisE_stack, mnsE_stack, Ia_flxs_t_stack, II_flxs_t_stack, Ia_exts_t_stack, II_exts_t_stack, IaF2mnF_stack, IaF2IaiF_stack, IIF2exF_stack, IIF2IaiF_stack, IaE2mnE_stack, IaE2IaiE_stack, IIE2exE_stack, IIE2IaiE_stack, exF2mnF_stack, IaiE2mnF_stack, exE2mnE_stack, IaiF2mnE_stack, Ia_flxs_stack, II_flxs_stack, Ia_exts_stack, II_exts_stack, fb_Ia_flxs_stack, fb_II_flxs_stack, fb_Ia_exts_stack, fb_II_exts_stack, mnsF_inlayer_stack, mnsF_hx_stack, mnsE_inlayer_stack, mnsE_hx_stack, mnsF_x_stack, mnsF_g_stack, mnsF_gpre_stack, mnsF_inlayerpre_stack, mnsE_x_stack, mnsE_g_stack, mnsE_gpre_stack, mnsE_inlayerpre_stack] ##### 230605
#                 return emg_stack, [afferents_ch, afferents_ch_ees, afferents, exsF_stack, IaisF_stack, mnsF_stack, exsE_stack, IaisE_stack, mnsE_stack, Ia_flxs_t_stack, II_flxs_t_stack, Ia_exts_t_stack, II_exts_t_stack, IaF2mnF_stack, IaF2IaiF_stack, IIF2exF_stack, IIF2IaiF_stack, IaE2mnE_stack, IaE2IaiE_stack, IIE2exE_stack, IIE2IaiE_stack, exF2mnF_stack, IaiE2mnF_stack, exE2mnE_stack, IaiF2mnE_stack, Ia_flxs_stack, II_flxs_stack, Ia_exts_stack, II_exts_stack, fb_Ia_flxs_stack, fb_II_flxs_stack, fb_Ia_exts_stack, fb_II_exts_stack, mnsF_inlayer_stack, mnsF_hx_stack, mnsE_inlayer_stack, mnsE_hx_stack, mnsF_x_stack, mnsF_g_stack, mnsF_gpre_stack, mnsF_inlayerpre_stack, mnsE_x_stack, mnsE_g_stack, mnsE_gpre_stack, mnsE_inlayerpre_stack, exsF_inlayer_stack, exsF_hx_stack, exsE_inlayer_stack, exsE_hx_stack, exsF_x_stack, exsF_g_stack, exsF_gpre_stack, exsF_inlayerpre_stack, exsE_x_stack, exsE_g_stack, exsE_gpre_stack, exsE_inlayerpre_stack, IaisF_inlayer_stack, IaisF_hx_stack, IaisE_inlayer_stack, IaisE_hx_stack, IaisF_x_stack, IaisF_g_stack, IaisF_gpre_stack, IaisF_inlayerpre_stack, IaisE_x_stack, IaisE_g_stack, IaisE_gpre_stack, IaisE_inlayerpre_stack] ##### 230605
                return emg_stack, [afferents_ch, afferents_ch_ees, afferents, exsF_stack, IaisF_stack, mnsF_stack, exsE_stack, IaisE_stack, mnsE_stack, Ia_flxs_t_stack, II_flxs_t_stack, Ia_exts_t_stack, II_exts_t_stack, IaF2mnF_stack, IaF2IaiF_stack, IIF2exF_stack, IIF2IaiF_stack, IaE2mnE_stack, IaE2IaiE_stack, IIE2exE_stack, IIE2IaiE_stack, exF2mnF_stack, IaiE2mnF_stack, exE2mnE_stack, IaiF2mnE_stack, Ia_flxs_stack, II_flxs_stack, Ia_exts_stack, II_exts_stack, fb_Ia_flxs_stack, fb_II_flxs_stack, fb_Ia_exts_stack, fb_II_exts_stack, mnsF_inlayer_stack, mnsF_hx_stack, mnsE_inlayer_stack, mnsE_hx_stack, exsF_inlayer_stack, exsF_hx_stack, exsE_inlayer_stack, exsE_hx_stack, IaisF_inlayer_stack, IaisF_hx_stack, IaisE_inlayer_stack, IaisE_hx_stack] ##### 230606